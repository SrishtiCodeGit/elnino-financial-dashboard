...
# ── Floating Menu button via components.html (scripts execute in iframe) ───────
components.html("""
<script>
(function() {
  try {
    ...
    function makeFAB() {
      if (D.getElementById('elnino-menu-fab')) return;

      var fab = D.createElement('div');
      fab.id = 'elnino-menu-fab';
      fab.title = 'Open / close navigation';
      fab.innerHTML =
        ...
        '</svg>' +
        '<span style="color:rgba(255,255,255,0.88);font-size:7px;font-weight:700;' +
        'letter-spacing:0.11em;font-family:Inter,system-ui,sans-serif;margin-top:3px">MENU</span>';
      ...
      fab.style.cssText =
        ...
        'width:44px;min-height:84px;' +
        'box-shadow:4px 0 22px rgba(29,78,216,0.55),0 0 0 1px rgba(59,130,246,0.3);' +
        'display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;' +
        'cursor:pointer;transition:width 0.18s ease,box-shadow 0.18s ease;user-select:none;';

      fab.addEventListener('mouseover', function() {
        this.style.width = '58px';
        this.style.boxShadow = '6px 0 28px rgba(29,78,216,0.7),0 0 0 1px rgba(59,130,246,0.45)';
      });
      fab.addEventListener('mouseout', function() {
        this.style.width = '44px';
        this.style.boxShadow = '4px 0 22px rgba(29,78,216,0.55),0 0 0 1px rgba(59,130,246,0.3)';
      });
    ...
    if (D.readyState === 'loading') {
      D.addEventListener('DOMContentLoaded', makeFAB);
    } else {
      makeFAB();
    }

    /* Recreate if Streamlit re-renders remove it */
    new MutationObserver(makeFAB).observe(D.body, {childList: true, subtree: false});

  } catch(e) { console.warn('Menu FAB:', e); }
})();
</script>
""", height=0)
